        ...
        {sessions?.length > 0 ? sessions.map((s) => (
          <div key={s.id} className={`session-card-full ${s.status === 'COMPLETED' ? 'completed-card' : ''}`}>
            <div className="card-top">
              <div className="status-label">{s.status || 'UNKNOWN'}</div>
              <span className="phase-label">{s.phase || 'General'}</span>
            </div>

            <h3 className="proc-name">{s.procedureName || 'Procedure Title'}</h3>

            <div className="meta-info">
              <div className="meta-item">
                <Calendar size={18} />
                <span>{s.scheduledDate ? new Date(s.scheduledDate).toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }) : 'Date TBD'}</span>
              </div>
              <div className="meta-item">
                <User size={18} />
                <span>Dr. {s.practitionerName || 'Assigned Vaidya'}</span>
              </div>
            </div>
            ...
            {s.feedback && (
              <div className="completed-feedback">
                <p><strong>Your Feedback:</strong> {s.feedback?.message || 'No feedback text provided.'}</p>
